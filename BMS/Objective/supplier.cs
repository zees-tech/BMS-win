    ...
    public partial class supplier : Form
    {
        ...
        private void Savebtn_Click(object sender, EventArgs e)
        {
            if (check_condition())
            {
                ...
                SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["SupplierDB"].ConnectionString);
                con.Open();
                string qurry = "insert into Supplier_T (Name, Alias, GSTIN,author_ID_fkey) values('" + nametxt.Text + "','" + aliastxt.Text + "','" + gstintxt.Text + "','" + Calculator.get_temp_data(Constant_And_Variable.Logger_id) + "')";
                SqlCommand cmd = new SqlCommand(qurry, con);
                try
                {
                    cmd.ExecuteNonQuery();
                    Ledger_Process.New_Ledger("Supplier_" + lastid_no());
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.ToString());
