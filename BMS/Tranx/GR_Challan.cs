...
namespace bms1
{
    public partial class GR_Challan : Form
    {
        public GR_Challan(string callas)
        {
            InitializeComponent();
            InitializeComponent();
        ...
        int lastbill_no()
        {
            ...
            con.Open();
            string qurry = "Select Id from GR_Challan_T ";
            SqlCommand cmd = new SqlCommand(qurry, con);
            int invo_no = 0;
            try
            {
        ...
        private void NavRec(int i = 0)
        {
            ...
            string qurry = "select * from " + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "GR_Challan_D_T where Invo_ID_fkey ='" + billidtxt.Text + "'";
            SqlCommand cmd = new SqlCommand(qurry, con);
        ...
        void fill_ds()
        {
            SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["TranxDB"].ConnectionString);
            con.Open();
            string qurry = "select * from "+Calculator.get_temp_data(Constant_And_Variable.Company_id)+"GR_Challan_T";
            SqlCommand cmd = new SqlCommand(qurry, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            try
            {
        ...
        private void savebtn_Click(object sender, EventArgs e)
        {
            if (check_condition())
            {
                ...
                string qurry = "insert into Pur_Challan_T (Com_ID_fkey, send_id_fkey, date_, discrate, author_ID_fkey) values('" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "','" + suppliercmbx.SelectedValue.ToString() + "','" + DateTime.Now.ToString("yyyyMMdd") + "','" + discrattxt.Text + "','" + emplbl.Text + "') ";
                ...
                try
                {
                    ...
                    Ledger_Process.Ledger_Update("Company_" + Calculator.get_temp_data(Constant_And_Variable.Company_id), "Supplier_" + suppliercmbx.SelectedValue.ToString(), "Credit", billidtxt.Text, Convert.ToDouble(grtottxt.Text), "Sarees " + qntytxt.Text + " recieved @ discount " + discrattxt.Text + "%");
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.ToString());
                ...
                {                    
                    qurry = "insert into GR_Challan_D_T (Invo_ID_fkey, item_no_fkey, rate, qnty) values('" + billidtxt.Text + "','" + itemdtdgv.Rows[n].Cells[0].Value + "','" + itemdtdgv.Rows[n].Cells[4].Value + "','" + itemdtdgv.Rows[n].Cells[3].Value + "') ";
                    ...
                    try
                    {
                        ...
                        Ledger_Process.Item_Update(comlbl.Text, "Item_" + itemdtdgv.Rows[n].Cells[0].Value.ToString(), "Debit", billidtxt.Text, Convert.ToDouble(itemdtdgv.Rows[n].Cells[3].Value.ToString()));
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.ToString());
