...
namespace bms1
{
    ...
    {
        public Debit_Note(string callas)
        {
            InitializeComponent();
        ...
        void fill_ds()
        {
            ...
            con.Open();
            string qurry = "select * from [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "Debit_Note_T]";
        ...
        private void NavRec(int i = 0)
        {
            if (ds.Rows.Count > 0)
            {
                ...
                dgvDelete();
                SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["TranxDB"].ConnectionString);
                string qurry = "select * from [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "Dbt_D_T] where Cre_ID_fkey ='" + noteidtxt.Text + "'";
                SqlCommand cmd = new SqlCommand(qurry, con);
                con.Open();
                try
                {
        ...
        private void addbtn_Click(object sender, EventArgs e)
        {
            if (dgvItemloaded.Rows.Count > 0)
            {
                ...
                SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["TranxDB"].ConnectionString);
                con.Open();
                string qurry = "Select * from [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "Bill_T] where Id=" + invo_notxt + "";
                SqlCommand cmd = new SqlCommand(qurry, con);
                cmd = new SqlCommand(qurry, con);
                try
                {
        ...
        int lastinvo_no()
        {
            SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["TranxDB"].ConnectionString);
            con.Open();
            string qurry = "Select Id from [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "Debit_Note_T] ";
            SqlCommand cmd = new SqlCommand(qurry, con);
            int invo_no = 0;
            try
            {
        ...
        private void savebtn_Click(object sender, EventArgs e)
        {
            if (check_condition())
            {
                ...
                string qurry = "insert into [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "Debit_Note_T] ( receiver_id_fkey, date_, discrate, author_ID_fkey, ) values('" + Cuscmbx.SelectedValue.ToString() + "','" + DateTime.Now.ToString("yyyyMMdd") + "','" + discrattxt.Text + "','" + emplbl.Text + "') ";
                ...
                try
                {
                    ...
                    Ledger_Process.Ledger_Update_GST(Calculator.get_temp_data(Constant_And_Variable.Company_id), Calculator.get_temp_data(Constant_And_Variable.Customer_id), "Debit", noteidtxt.Text, Convert.ToDouble(tottxt.Text));
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.ToString());
