...
namespace bms1
{
    public partial class purchase : Form
    {
        ...
        int lastbill_no()
        {
            ...
            con.Open();
            string qurry = "Select Id from [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "]Bill_T ";
            SqlCommand cmd = new SqlCommand(qurry, con);
            int invo_no = 0;
            try
            {
        ...
        void cal(int n, int sign)
        {
            ...
            con.Open();
            string qurry = "Select State from Adres_T where addri_id_fkey=" + suppliercmbx.SelectedValue + "";
            SqlCommand cmd = new SqlCommand(qurry, con);
            string sstate = "";
            try
            {
        ...
        string ret_name(string L_id)
        {

            SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["SupplierDB"].ConnectionString);
            string qurry = "select * from Supplier_T where Id ='" + L_id + "'";
            SqlCommand cmd1 = new SqlCommand(qurry, con);
            con.Open();
        ...
        private void NavRec(int i = 0)
        {
            ...
            SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["TranxDB"].ConnectionString);
            con.Open();
            string qurry = "select * from [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "Bill_D_T] where Bill_ID_fkey ='" + billidtxt.Text + "'";
            SqlCommand cmd = new SqlCommand(qurry, con);
            Edit();
            ...
            suppliercmbx.Text = ret_name(ds.Rows[i][3].ToString());
            dgvDelete();
            try
            {
        ...
        void fill_ds()
        {
            SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["TranxDB"].ConnectionString);
            con.Open();
            string qurry = "select * from [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "Bill_T]";
            SqlCommand cmd = new SqlCommand(qurry, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            try
            {
        ...
        private void savbtn_Click(object sender, EventArgs e)
        {
            if (check_condition())
            {
                ...
                string qurry = "insert into [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "Bill_T] (Com_ID_fkey, send_id_fkey, date_, discrate, author_ID_fkey) values('" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "','" + suppliercmbx.SelectedValue.ToString() + "','" + DateTime.Now.ToString("yyyyMMdd") + "','" + discrattxt.Text + "','" + emplbl.Text + "') ";
                ...
                while (n < itemdtdgv.Rows.Count)
                {
                    ...
                    qurry = "insert into [" + Calculator.get_temp_data(Constant_And_Variable.Company_id) + "Bill_D_T] (Bill_ID_fkey, item_no_fkey, rate, qnty) values('" + billidtxt.Text + "','" + itemdtdgv.Rows[n].Cells[0].Value + "','" + itemdtdgv.Rows[n].Cells[4].Value + "','" + itemdtdgv.Rows[n].Cells[3].Value + "') ";
